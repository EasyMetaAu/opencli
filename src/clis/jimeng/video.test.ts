...
import { buildDraftContent, validateVideoParams } from './video.js';
...
describe('validateVideoParams', () => {
  it('allows text-to-video (no images)', () => {
    expect(() => validateVideoParams({ refImagePath: '', firstFramePath: '', lastFramePath: '' })).not.toThrow();
  });

  it('allows ref-image only', () => {
    expect(() => validateVideoParams({ refImagePath: 'ref.png', firstFramePath: '', lastFramePath: '' })).not.toThrow();
  });

  it('allows first-frame only', () => {
    expect(() => validateVideoParams({ refImagePath: '', firstFramePath: 'first.png', lastFramePath: '' })).not.toThrow();
  });

  it('allows first-frame + last-frame', () => {
    expect(() => validateVideoParams({ refImagePath: '', firstFramePath: 'first.png', lastFramePath: 'last.png' })).not.toThrow();
  });

  it('rejects --last-frame without --first-frame', () => {
    expect(() => validateVideoParams({ refImagePath: '', firstFramePath: '', lastFramePath: 'last.png' }))
  ...
  it('rejects --ref-image with --first-frame', () => {
    expect(() => validateVideoParams({ refImagePath: 'ref.png', firstFramePath: 'first.png', lastFramePath: '' }))
      .toThrow(ArgumentError);
  });

  it('rejects --ref-image with --last-frame', () => {
    expect(() => validateVideoParams({ refImagePath: 'ref.png', firstFramePath: '', lastFramePath: 'last.png' }))
      .toThrow(ArgumentError);
  });

  it('rejects --ref-image with both --first-frame and --last-frame', () => {
    expect(() => validateVideoParams({ refImagePath: 'ref.png', firstFramePath: 'first.png', lastFramePath: 'last.png' }))
      .toThrow(ArgumentError);
  });
});
...
describe('buildDraftContent — ref-image mode', () => {
  const opts = { ...BASE_OPTS, refImage: FAKE_IMG };
